...
from mindprompt.utils.utils import signature
from mindprompt.config import convert_cfg_to_dict
from mindprompt.data_utils import InputFeatures, InputExample


class Template(nn.Cell):
    r'''
...
class Verbalizer(nn.Cell):
    ...
    def _match_label_words_to_label_ids(self,
                                        label_words):  # TODO newly add function after docs written # TODO rename this function
        """
    ...
    def register_calibrate_logits(self, logits: Tensor):
        r"""
    ...
    def gather_outputs(self, outputs):
        ...
        Args:
            outputs : The output from the pretrained language model.
    ...
    @staticmethod
    def aggregate(label_words_logits: Tensor) -> Tensor:
    ...
    def normalize(self, logits: Tensor) -> Tensor:
        ...
        return ops.softmax(logits.reshape(batch_size, -1), dim=-1).reshape(*logits.shape)
    ...
    def handle_multi_token(self, label_words_logits, mask):
        ...
        if self.multi_token_handler == "first":
            # 修改
            # label_words_logits = label_words_logits.select(axis=-1, index=0)
            label_words_logits = label_words_logits.index_select(axis=-1, index=Tensor([0])).squeeze(axis=-1)
        elif self.multi_token_handler == "max":
            label_words_logits = label_words_logits - 1000 * (1 - mask.unsqueeze(0))
            label_words_logits = label_words_logits.max(axis=-1).values
        elif self.multi_token_handler == "mean":
            label_words_logits = (label_words_logits * mask.unsqueeze(0)).sum(axis=-1) / (
                        mask.unsqueeze(0).sum(axis=-1) + 1e-15)
        else:
            ...
            if not hasattr(config, "file_path"):
                ...
                if (not hasattr(config, "label_words") or config.label_words is None) and config.file_path is not None:
                    ...
                    verbalizer.from_file(config.file_path, config.choice)
                              ...
                              "label_words") and config.label_words is not None) and config.file_path is not None:
                    raise RuntimeError("The text can't be both set from `text` and `file_path`.")
        return verbalizer
