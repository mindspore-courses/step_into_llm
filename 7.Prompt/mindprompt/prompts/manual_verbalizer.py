...
from mindprompt import Verbalizer
...
class ManualVerbalizer(Verbalizer):
    ...
    def generate_parameters(self) -> List:
        ...
        for words_per_label in self.label_words:
            ...
            for word in words_per_label:
                ...
                ids_per_label.append(ids.ids)
            all_ids.append(ids_per_label)

        max_len = max([max([len(ids) for ids in ids_per_label]) for ids_per_label in all_ids])
        max_num_label_words = max([len(ids_per_label) for ids_per_label in all_ids])
        words_ids_mask = ops.zeros((max_num_label_words, max_len))
        ...
        words_ids_tensor = Tensor(words_ids)
        words_ids_mask = Tensor(words_ids_mask)
        self.label_words_ids = Parameter(words_ids_tensor, requires_grad=False)
        self.words_ids_mask = Parameter(words_ids_mask, requires_grad=False)  # A 3-d mask
        self.label_words_mask = Parameter(ops.clamp(words_ids_mask.sum(axis=-1), max=1), requires_grad=False)
    ...
    def project(self,
        ...
        Args:
            logits (:obj:`Tensor`): The original logits of label words.

        Returns:
            :obj:`Tensor`: The normalized logits of label words
        """
        ...
        label_words_logits -= 10000 * ops.sub(1, self.label_words_mask)
    ...
    def process_logits(self, logits: Tensor, **kwargs):
        ...
        Args:
            logits (:obj:`Tensor`): The original logits.

        Returns:
            (:obj:`Tensor`): The final processed logits over the labels (classes).
        """
        # project
        ...
        if self.post_log_softmax:
            ...
            # convert to logits
            label_words_logits = ops.log(label_words_probs + 1e-15)

        # aggregate
        label_logits = self.aggregate(label_words_logits)
        return label_logits

    def normalize(self, logits: Tensor) -> Tensor:
        """
        ...
        return ops.softmax(logits.reshape(batch_size, -1), axis=-1).reshape(*logits.shape)

    def aggregate(self, label_words_logits: Tensor) -> Tensor:
        r"""Use weight to aggregate the logits of label words.

        Args:
            label_words_logits(:obj:`Tensor`): The logits of the label words.

        Returns:
            :obj:`Tensor`: The aggregated logits from the label words.
        """
    ...
    def calibrate(self, label_words_probs: Tensor, **kwargs) -> Tensor:
        ...
        Args:
            label_words_probs (:obj:`Tensor`): The probability distribution of the label words with the shape of [``batch_size``, ``num_classes``, ``num_label_words_per_class``]

        Returns:
            :obj:`Tensor`: The calibrated probability of label words.
        """
