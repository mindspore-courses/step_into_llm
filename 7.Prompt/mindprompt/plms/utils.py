...
from mindprompt.utils import round_list
        ...
        for key in encoder_inputs:
            if key == "input_ids":
                with warnings.catch_warnings():
                    ...
                    encoder_inputs[key] = self.build_inputs_with_special_tokens(encoder_inputs[key])
            else:
                special_tokens_mask = np.array(self.get_special_tokens_mask(encoder_inputs[key]))
                with_special_tokens = np.array(self.build_inputs_with_special_tokens(encoder_inputs[key]))
