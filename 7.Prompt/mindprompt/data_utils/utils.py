...
class InputFeatures(dict):
    ...
    mindspore's default_collate in DataLoader.
    Also a :py:meth:`to_tensor()` method is build to convert the values into Tensor for mindspore's input.
    ...
    def __init__(self,
                input_ids: Optional[Union[List, Tensor]] = None,
                inputs_embeds: Optional[Tensor] = None,
                attention_mask: Optional[Union[List[int], Tensor]] = None,
                token_type_ids: Optional[Union[List[int], Tensor]] = None,
                label: Optional[Union[int, Tensor]] = None,
                decoder_input_ids: Optional[Union[List, Tensor]] = None,
                decoder_inputs_embeds: Optional[Tensor] = None,
                soft_token_ids: Optional[Union[List, Tensor]] = None,
                past_key_values: Optional[Tensor] = None,  # for prefix_tuning
                loss_ids: Optional[Union[List, Tensor]] = None,
    ...
    def to_tensor(self, device: str = 'cuda'):
        """inplace operation, convert all tensorable features into :obj:`Tensor`"""
        for key in self.tensorable_keys:
            value = getattr(self, key)
            if value is not None:
                setattr(self, key, Tensor(value))
        return self
    ...
    def collate_fct(batch: List):
        ...
        for key in elem:
            if key == "encoded_tgt_text":
                ...
                try:
                    batches = [d[key] for d in batch]
                    return_dict[key] = ops.stack(batches, 0)
                except:
